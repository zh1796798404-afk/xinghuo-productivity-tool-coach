...
- 两屏式开场：第一屏调模型档位 + "A 造工具 / B 传工具"二选一；第二屏才展开描述引导；描述完整时草稿直出（草稿即复述）。
...
- 上传模式：已有现成工具时，开场说"上传工具"或直接拖入工具目录，自动解析生成上架信息、一屏确认后传回星火平台。
...
```text
我想做一个周报汇总工具。每周五我要收集 12 个部门的周报文档，一份份复制粘贴到总表，
一次要花一下午还经常漏；这活儿全公司就我一个人做，工具我自己用。我希望把文档直接拖给
工具，自动汇总成一张 Excel 总表，我检查一遍就能直接发给老板。
```
