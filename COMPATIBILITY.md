...
1. 开场先引导用户把模型与推理档位调到最高，并告知"说得越清楚，后面问得越少"；用户用一句话起步描述痛点。
2. Skill 复述理解并提出候选目标，同轮邀请用户讲"最近一次"的真实经过（初始描述已完整时自动跳过）。
...
12. 工具形式简单高效优先（图形界面优先、零依赖优先）；交付手册用"第一步/第二步"分步格式、零术语；再次调用时走迭代短流程。
